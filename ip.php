...
// Om X-Forwarded-For är satt (kan innehålla flera adresser)
if ($xfwd) {
...
// Om ingen giltig IP hittats via X-Forwarded-For, använd REMOTE_ADDR
...
// För backwards-kompatibilitet sätter vi $IP till IPv4 om den finns, annars IPv6
if ($ipv4) {
    $IP = $ipv4;
} elseif ($ipv6) {
    $IP = $ipv6;
...
// Hämta host baserat på IPv4 (om finns) annars IPv6
if ($ipv4) {
